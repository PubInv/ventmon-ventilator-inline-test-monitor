...
2020, by beginning over 100 open source ventilator project\cite{COVID19VENTLIST}.
...
in hopes that smaller modules may be reusable even by those who have not use for the VentMon specifically.

\begin{enumerate}
\item The main firmware is written in the Arudino environment, and though targeting and ESP32 could easily apply to other boards. This core firmware manages reading the various senosrs, and
outputs its data both on built in OLED (for function testing), and, more importantly, via the PIRDS standard to a public data lake where a separate module renders it with a display appropriate
for a clinician.
\item Although our current focus is on a printed circuit board version of the VentMon, we report here a system that can be made by an amateur using only commercial off-the-shelf (COTS) parts.
...
{\em The VentMon T0.3 is not meant to be used on human patients. } The COTS version uses firmware v0.2 tagged as such\cite{VentMon02}. This version is somewhat incompatible
...
a respriation assistance device being tested (such as a pandemic ventilator or CPAP machine) and a simply physical test lung which
models compliance and resistance of the human lung.
...
\item At startup, by default configuration (it can be enabled or disabled), the VentMon tries to see if their is a cable connected to the Ethernet RJ45 jack.
  ...
  Your data can be viewed in real time by pointing a browser at the URL. Your IP address will appear near the top of the list; by clicking on ``Breath Plot'' you will be able to see a complete clinical display.
...
Pressing the ``C'' button on the VentMon send a special signal to the PIRDS logger to ``rotate the log'' from this IP address. At the public data lake the current file will be copied to a file which a date and timestamp.
...
and modify the firmware. The usage will likely be the same, though at the time of this writing some features
