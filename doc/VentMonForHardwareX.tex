...
The primary use of the VentMon is to develop to and verify that a rapidly manufactured ventilator meets required specifications, such as those published by the UK MHRA\cite{mhra2020specification}.

Because the VentMon produces pressure and flow graphs at about 25 samples per seconds which -- fast compared to
...
match the breath, mechanical ventilation, or changes in the resistance -- compliance -- of the test lung being used.
...
Figure \ref{fig:ventdisplay} is a screenshot of VentDisplay rendering data from the VentMon with a human (RLR) breathing normally into a plastic test lung for 10 seconds. Although the VentDisplay and VentMon can be used independently, VentDisplay demonstrates the data VentMon collects. The top trace is a pressure trace. The next shows flow. These curves are generated from samples published into the public data lake. The third trace is the ``event'' curve which shows a number of computed values and events published from the VentMon, such as the humidity, FiO$_2$, and temperature. Additionally, the beginning and ending of each breath is detected, and total volume of the breath computed by integrating the flow.

Such calculations allows us to present the numbers on the right of the display, which are typical of a commercial ventilator display, but are completely independent of knowledge of the ventilator. This allows a would-be ventilator's performance to be tested as part of a validation and verification plan. Features such as rise and fall time (at the bottom on the right) are particularly important. The most basic function of VentDisplay is to dynamically report the respiration rate, peak pressure, minimum pressure, tidal volume and minute volume, and I:E ratio.

%%%% COME BACK

%Overall, our software ecosystem and hardware offers three core benefits over other options; it measures pressure rise time, 

 Our software measures pressure rise time which we have argued is an under-appreciated performance characteristic of rapidly manufactured emergency ventilators \cite{schulz2020importance}. This software has been shown to be valuable, although it is dependent on accurate computation of the beginning and ending of a breath, which can be subtle in some situations.

The flow and differential pressure sensors used in the VentMon require no calibration. The oxygen sensor is self-calibrated and good enough for accurately measuring FiO$_2$ mixing in mechanical ventilation. Flow was checked with a graduated syringe and found to be quite accurate.

The VentMon published data in the Public Invention Respiration Data Standard format \cite{PIRDS}, which we created to provide a standard for multiple teams to cooperate. A consequence of using this standard and IoT cloud-based software is the ability to support a distributed engineering team, which has been frequently required by the COVID-19 social distancing measures. We aspire to someday support telemedicine as suggested by others \cite{rehm2018development}. The VentDisplay software allows export of the JSON binding of the PIRDS data to allow other programs to process if it if desired.


In fact we hope the VentMon will evolve into an open-source component that can be freely integrated into a pandemic ventilator or simply ``bolted on'' to be used to control ventilators or to monitor human patients. Although it can effectively test ventilation equipment today it will require regulatory approval before it can be recommended for use on patients. Before that can happen, the VentMon must progress beyond a number of current limitations:
\begin{enumerate}
\item 
The current device is not autoclavable or easy to sanitize. Use on a human patient would therefore require it to be disposed of immediately.
\item 
Our software has not been subject to rigorous Quality Assurance (QA) and Risk Analysis (RA) as requested by the FDA. We have argued\cite{ecosystem} for an ecosystem in which design teams such as Public Invention produce extensive documentation including  QA/RA documentation to be reused by firms that become medical device manufactures incorporating open-source designs.
\item 
Further mechanical design is required to increase the robustness of the physical device.
\item
Although the current data lake and software records all data coming from a VentMon device for many days or weeks -- allowing longitudinal study of ventilator reliability -- we have not yet written software to allow this data to be analyzed or accessed across long periods of time.
\end{enumerate}
