...
//      setWiFiEnabledFromEEPROM(0);
  ...
  udpclient = (UDP *) (wudpclient_good ? (UDP *) &wudpclient : NULL);
...
String SendHTML(){
  ...
  ptr +="<form action=\"/submit\">\n";
  ptr +="<label for=\"wifi\">WiFi:</label>\n";
  ptr +="<input type=\"text\" id=\"wifi\" name=\"fname\"><br><br>\n";
  ptr +="<label for=\"pass\">Password:</label>\n";
  ptr +="<input type=\"text\" id=\"pass\" name=\"pass\"><br><br>\n";
...
String SendHTMLSuccess(){
  ...
  ptr +="<p>WiFi SSID and password updated!</p>\n";
...
void handle_OnWifi() {
  server.send(200, "text/html", SendHTMLSuccess());
}
...
//  EEPROM.begin(32+120+1+1);
  ...
  ventMonPrefs.begin("VentMonPrefs", true); // RO mode
  
  bool tpInit = ventMonPrefs.isKey("nvsInit");       // Test for the existence of the "already initialized" key.
  ...
  if (tpInit == false) {
      ...
      ventMonPrefs.end();                             // close the namespace in RO mode and...
      ventMonPrefs.begin("VentMonPrefs", false); // RW mode        //  reopen it in RW mode.


      // The .begin() method created the "VentmonPrefs" namespace and since this is our
      // first-time run we will create our keys and store the initial "factory default" values.
      ventMonPrefs.putString("ssid", "Ventmon");
      ventMonPrefs.putString("wifiPass", "NULL");
      ventMonPrefs.putBool("wifiOn", true);
      ventMonPrefs.putBool("wifiAP", true);
      ventMonPrefs.putBool("bluetoothOn", false);

      ventMonPrefs.putBool("nvsInit", true);          // Create the "already initialized" key and store a value.

      // The "factory defaults" are created and stored so...
      ventMonPrefs.end();                             // Close the namespace in RW mode and...
      ventMonPrefs.begin("VentMonPrefs", true); // RO mode    //  reopen it in RO mode so the setup code
                                                  //  outside this first-time run 'if' block
                                                  //  can retrieve the run-time values
                                                  //  from the "STCPrefs" namespace.
   }

   String ssid = ventMonPrefs.getString("ssid");
   String wifiPassword = ventMonPrefs.getString("wifiPass");
   bool wifiOn = ventMonPrefs.getBool("wifiOn");
   bool wifiAP = ventMonPrefs.getBool("wifiAP");
   bool bluetoothOn = ventMonPrefs.getBool("bluetoothOn");
...
//  displayPrintScroll("Sensor test complete!");
  ...
  if (ventMonPrefs.getBool("wifiAP"))
  {
    Serial.println("Wifi AP");
    // Create a WiFi access point. This is the factory default behaviour so the user
    // can connect to the VentMon and set their own WiFi credentials.
    WiFi.mode(WIFI_AP);
    WiFi.softAP("VentMon", NULL);
    Serial.print("[+] AP Created with IP Gateway ");
    myIP = static_cast<String>(WiFi.softAPIP());
    Serial.println(myIP);
    delay(100);
