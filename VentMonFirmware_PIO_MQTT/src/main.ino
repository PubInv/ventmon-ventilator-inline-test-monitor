...
// Internal pressure values are stored as tenths of cmH2O.
// 40.0 cmH2O => 400 tenths of cmH2O.
const signed long OVER_PRESSURE_ALARM_LIMIT_10THS_CM_H2O = 400; 
...
void publishOverPressureToKrake(signed long pressureTenthsCmH2O) {
  ...
  float cmH2O = fabs((float)pressureTenthsCmH2O) / 10.0f;
  char onLineMsg[96] = "";
  snprintf(onLineMsg, sizeof(onLineMsg), "a3 Inspiratory OverPressure: %.1f cmH2O", cmH2O);
...
const char* mqttMeasurementName(char t) {
  switch (t) {
    ...
    case 'D': return "differential_pressure";
    case 'P': return "absolute_pressure";
    case 'T': return "temperature";
    case 'H': return "humidity";
    case 'A': return "altitude";
    case 'G': return "gas_resistance";
    default:  return "unknown";
  }
}

const char* mqttLocationName(char loc) {
  switch (loc) {
    case 'I': return "inspiratory";
    case 'E': return "expiratory";
    case 'B': return "ambient";
    default:  return "unknown";
  }
}

const char* mqttUnitForType(char t) {
  switch (t) {
    case 'F': return "slm";
    case 'O': return "%";
    case 'D': return "cmH2O";
...
float mqttScaledValue(char t, signed long val) {
  switch (t) {
    ...
    case 'D': return ((float)val) / 10.0f;   // internal: mmH2O, MQTT: cmH2O
    case 'P': return ((float)val) / 10.0f;   // internal: mmH2O, MQTT: cmH2O
    case 'T': return ((float)val) / 100.0f;  // internal: C * 100
    case 'H': return ((float)val) / 100.0f;  // internal: %RH * 100
...
void fillPolishedMqttMeasurement(char e, char t, char loc, unsigned short int n, unsigned long ms, signed long val,
  ...
  JsonDocument doc;
  doc["event"] = String(e);
  doc["measurement"] = mqttMeasurementName(t);
  doc["type"] = String(t);        // keep original PIRDS type for compatibility
  doc["location"] = mqttLocationName(loc);
  doc["loc"] = String(loc);       // keep original PIRDS loc for compatibility
  doc["num"] = n;
  doc["value"] = scaledValue;
  doc["raw_value"] = val;
...
void outputMeasurement(char e, char t, char loc, unsigned short int n, unsigned long ms, signed long val) {
  ...
  char mqttBuff[384];
  fillPolishedMqttMeasurement(e, t, loc, n, ms, val, mqttBuff, sizeof(mqttBuff));

  if (OUTPUT_DATA_TO_SERIAL) {
    Serial.print(mqttBuff);
    Serial.println();
  }
...
// Network setup summary is intentionally temporary.
// It appears during WiFiManager setup and for 30 seconds after setup finishes,
// then the full OLED area returns to the normal VentMon display.
#define NETWORK_STATUS_BAR_Y 24
#define NETWORK_STATUS_BAR_H 8
#define MAIN_DISPLAY_MAX_Y display.height()
#define NETWORK_SUMMARY_AFTER_SETUP_MS 30000UL
unsigned long networkStatusVisibleUntilMs = 0;

bool networkStatusSummaryIsVisible() {
  return networkStatusVisibleUntilMs != 0 && (long)(networkStatusVisibleUntilMs - millis()) > 0;
}

void renderNetworkStatusBar() {
  if (!found_display || !networkStatusSummaryIsVisible()) return;
  display.fillRect(0, NETWORK_STATUS_BAR_Y, display.width(), NETWORK_STATUS_BAR_H, 0);
  display.setTextSize(1);
  display.setTextColor(SH110X_WHITE);
  display.setCursor(0, NETWORK_STATUS_BAR_Y);
  display.print(F("W:"));
  display.print(networkServiceWifiConnected() ? F("1") : F("0"));
  display.print(F(" B:"));
  display.print(networkServiceMqttConnected() ? F("1") : F("0"));
  display.print(F(" "));
  display.print(networkServiceIpAddress());
}

void showNetworkSetupCompleteSummary() {
  if (!found_display) return;
  networkStatusVisibleUntilMs = millis() + NETWORK_SUMMARY_AFTER_SETUP_MS;

  display.clearDisplay();
  display.setTextSize(1);
  display.setTextColor(SH110X_WHITE);
  display.setCursor(0, 0);
  display.print(F("Network ready"));
  display.setCursor(0, ROW_HEIGHT);
  display.print(F("WiFi:"));
  display.print(networkServiceWifiConnected() ? F("OK") : F("OFF"));
  display.print(F(" MQTT:"));
  display.print(networkServiceMqttConnected() ? F("OK") : F("OFF"));
  display.setCursor(0, ROW_HEIGHT * 2);
  display.print(networkServiceIpAddress());
  display.display();
}
...
#else
// Keep bottom 8 px free for the always-visible network status bar.
#define GRAPH_Y_PIXELS 24
#endif
...
void output_I_DPRES() {
    ...
    delay(500);
    if (abs(display_max_pressure) >= OVER_PRESSURE_ALARM_LIMIT_10THS_CM_H2O) {
        // tell the Krake
...
void output_I_ADPRES() {
    ...
    if (internal_pressure != LONG_MIN) {
      ...
      delay(500);
      if (abs(diff_pressure) >= OVER_PRESSURE_ALARM_LIMIT_10THS_CM_H2O) {
        // tell the Krake
        Serial.print("OVER_PRESSSURE!\n");
        publishOverPressureToKrake(diff_pressure);
      }
...
void showNetworkStatusOnDisplay(const char *line1, const char *line2) {
  if (!found_display) {
    return;
  }
  // During blocking WiFiManager setup, show clear instructions on the display.
  // After setup completes, the final network summary remains visible for 30 seconds.
  networkStatusVisibleUntilMs = millis() + NETWORK_SUMMARY_AFTER_SETUP_MS;
  display.clearDisplay();
  display.setTextSize(1);
  display.setTextColor(SH110X_WHITE);
  display.setCursor(0, 0);
  display.print(line1 ? line1 : "Network");
  display.setCursor(0, ROW_HEIGHT);
...
void setup() {
  ...
  // Network layer: WiFiManager captive portal, OTA, web visualizer, and configurable MQTT topics.
  // This callback makes blocking WiFiManager states visible on the OLED/LCD display.
  networkServiceSetStatusCallback(showNetworkStatusOnDisplay);
  networkServiceBegin();
  showNetworkSetupCompleteSummary();
  need_to_configure = false;
  Serial.println("Network service started. Open the device IP in a browser for the visualizer and MQTT topic settings.");
